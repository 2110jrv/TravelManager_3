...
## Savepoint 040 compact item editor JSON import
...
The editor exposes the important item payload fields already supported by full details and PDF output. Save still preserves unknown and unedited fields, including unknown fields loaded from JSON, stamps `UpdatedAt`, `ModifiedAt`, and `updatedAt`, increments `Version`, marks the item locally changed, and queues Supabase sync. Completion fields remain display-only in this modal because the checkmark workflow owns `Completed`, `CompletedAt`, and `CompletedByRole`.

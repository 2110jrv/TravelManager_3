-- Backfill-only HMAC lookup for existing PBKDF2 PINs. Never replaces pin_hash.
alter table public.av_app_access add column if not exists pin_lookup_hmac text;
create unique index if not exists av_app_access_active_pin_lookup_hmac_uidx
  on public.av_app_access(pin_lookup_hmac)
  where access_status='ACTIVE' and pin_lookup_hmac is not null;
...
begin
  ...
  if p_action='set_pin' then
    if coalesce(p_access_status,'ACTIVE')='ACTIVE' and p_pin_lookup_hmac is not null and exists(
      select 1 from public.av_app_access a where a.pin_lookup_hmac=p_pin_lookup_hmac and a.access_status='ACTIVE' and a.user_id<>p_target_user_id
    ) then raise exception 'AV_PIN_ALREADY_ASSIGNED'; end if;
    insert into public.av_app_access(user_id,access_status,role,pin_hash,pin_lookup_hmac,pin_changed_at,created_by_admin)
      values(p_target_user_id,coalesce(p_access_status,'ACTIVE'),coalesce(p_role,'VIEWER'),p_pin_hash,p_pin_lookup_hmac,now(),auth.uid())
    on conflict(user_id) do update set pin_hash=excluded.pin_hash,pin_lookup_hmac=excluded.pin_lookup_hmac,pin_changed_at=now(),
      access_status=coalesce(p_access_status,public.av_app_access.access_status),role=coalesce(p_role,public.av_app_access.role),updated_at=now();
...
$$;
revoke all on function public.av_admin_private_access_action_v3(uuid,uuid,text,text,text,text,text,jsonb) from public;
grant execute on function public.av_admin_private_access_action_v3(uuid,uuid,text,text,text,text,text,jsonb) to authenticated;

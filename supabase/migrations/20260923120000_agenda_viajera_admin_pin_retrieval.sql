-- Reversible PIN storage for ADMIN-only recovery. Plaintext is never stored.
alter table public.av_app_access add column if not exists pin_encrypted text;
alter table public.av_app_access add column if not exists pin_iv text;
alter table public.av_app_access add column if not exists pin_key_version text;
...
begin
  ...
  if p_action='set_pin' then
    ...
    insert into public.av_app_access(user_id,access_status,role,pin_hash,pin_lookup_hmac,pin_encrypted,pin_iv,pin_key_version,pin_changed_at,created_by_admin,failed_pin_attempts,pin_locked_until)
      values(p_target_user_id,coalesce(p_access_status,'ACTIVE'),coalesce(p_role,'VIEWER'),p_pin_hash,p_pin_lookup_hmac,p_pin_encrypted,p_pin_iv,p_pin_key_version,now(),auth.uid(),0,null)
    on conflict(user_id) do update set pin_hash=excluded.pin_hash,pin_lookup_hmac=excluded.pin_lookup_hmac,pin_encrypted=excluded.pin_encrypted,pin_iv=excluded.pin_iv,pin_key_version=excluded.pin_key_version,pin_changed_at=now(),access_status=coalesce(p_access_status,public.av_app_access.access_status),role=coalesce(p_role,public.av_app_access.role),failed_pin_attempts=0,pin_locked_until=null,updated_at=now();
...
create or replace function public.av_admin_record_pin_view(p_trip_id uuid,p_target_user_id uuid)
returns jsonb language plpgsql security definer set search_path=public as $$
begin
  if auth.uid() is null or not exists(select 1 from public.av_trip_memberships m where m.trip_id=p_trip_id and m.user_id=auth.uid() and m.role='ADMIN' and m.status='ACTIVE') then raise exception 'AV_ADMIN_REQUIRED'; end if;
  insert into public.av_audit_issues(trip_id,code,severity,repair_class,entry_ids,message) values(p_trip_id,'ADMIN_VIEWED_PIN','INFO','MANUAL',jsonb_build_array(p_target_user_id),'Administrator viewed a user PIN.');
  return jsonb_build_object('ok',true);
end;
$$;
revoke all on function public.av_admin_private_access_action_v4(uuid,uuid,text,text,text,text,text,text,text,text,jsonb),public.av_admin_record_pin_view(uuid,uuid) from public;
grant execute on function public.av_admin_private_access_action_v4(uuid,uuid,text,text,text,text,text,text,text,text,jsonb),public.av_admin_record_pin_view(uuid,uuid) to authenticated;

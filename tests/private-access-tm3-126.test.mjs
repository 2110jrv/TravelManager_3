...
test('private login requires managed PIN and exposes no signup',()=>{
  assert.match(main,/name="pin"/);assert.match(main,/signInWithPin/);assert.match(main,/Si olvidaste tu PIN, comunícate con Jonathan para que te asigne un nuevo PIN de acceso\./);assert.doesNotMatch(main,/name="email"|name="password"|forgot-password|send-own-reset|data-reset-request|data-recovery-password/);assert.doesNotMatch(main,/Crear cuenta/);assert.doesNotMatch(legacy,/Crear cuenta/);assert.match(legacy,/const ACCESS_PINS = Object\.freeze\(\{\}\);/);
});

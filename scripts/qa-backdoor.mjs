...
const URL='https://2110jrv.github.io/TravelManager_3/';
...
const cleanup=async()=>{
  ...
  await page.getByRole('heading',{name:'Usuarios y permisos'}).waitFor({state:'visible',timeout:10000});
  for(const name of [adminNameOf(suffix),nameOf(suffix)]){
    const row=page.locator('[data-user-row]').filter({hasText:name});
    if(await row.count()){
...
try{
  ...
  const context=await browser.newContext();page=await context.newPage();page.on('pageerror',()=>errors.push('pageerror'));page.on('console',message=>{if(message.type()==='error')errors.push('console')});
  const go=async()=>{await page.goto(`${URL}?qa-backdoor=${Date.now()}`,{waitUntil:'domcontentloaded'});await page.locator('input[name="pin"]').waitFor({state:'visible',timeout:30000})};
  ...
  const backdoor=async()=>{await go();await login(masterPin);await page.locator('[data-admin-user-form]').waitFor({state:'visible',timeout:30000});backdoorLoggedIn=true;await page.locator('[data-role-permission]').first().waitFor({state:'visible',timeout:30000});if(await page.locator('.bottom-nav').count())throw Error('backdoor exposed app navigation')};
  const addUser=async(name,pin,role)=>{await page.getByRole('button',{name:'+ Añadir usuario'}).click();const form=page.locator('[data-admin-user-form]');await form.locator('[name="displayName"]').fill(name);await form.locator('[name="pin"]').fill(pin);await form.locator('[name="pinConfirm"]').fill(pin);await form.locator('[name="role"]').selectOption(role);await form.locator('[name="status"]').selectOption('ACTIVE');await form.locator('[name="tripAccess"]').selectOption(TRIP_ID);await form.getByRole('button',{name:'Guardar'}).click();await page.getByText(name,{exact:true}).waitFor({state:'visible',timeout:30000})};
  const setChatPermission=async enabled=>{const checkbox=page.locator('[data-role-permission][data-role="TRAVELER"][data-permission="writeChat"]');await checkbox.waitFor({state:'visible',timeout:30000});if(await checkbox.isChecked()!==enabled){await checkbox.click();await page.waitForTimeout(500);await checkbox.waitFor({state:'visible'});if(await checkbox.isChecked()!==enabled)throw Error('permission update did not persist')}};
  ...
  await backdoor();const adminRow=page.locator('[data-user-row]').filter({hasText:adminNameOf(suffix)});page.once('dialog',dialog=>dialog.accept());await adminRow.getByRole('button',{name:'Borrar'}).click();await adminRow.waitFor({state:'detached',timeout:30000});adminCreated=false;const qaRow=page.locator('[data-user-row]').filter({hasText:nameOf(suffix)});page.once('dialog',dialog=>dialog.accept());await qaRow.getByRole('button',{name:'Borrar'}).click();await qaRow.waitFor({state:'detached',timeout:30000});qaCreated=false;await logout();
